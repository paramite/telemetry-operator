...
func (r *CeilometerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	defer func() {
		...
		if instance.CeilometerStatus.Conditions.AllSubConditionIsTrue() {
			...
			instance.CeilometerStatus.Conditions.MarkUnknown(
	...
	if instance.CeilometerStatus.Conditions == nil {
...
func (r *CeilometerReconciler) reconcileCeilometer(
	...
	Log.Info(fmt.Sprintf("Reconciled Service '%s' successfully", ceilometer.ServiceName))
	return ctrl.Result{}, nil
}
...
func (r *CeilometerReconciler) reconcileKSM(
	...
	// create the service
	ssDef, tlsConfDef, err := availability.KSMStatefulSet(instance, serviceLabels)
	if err != nil {
		return ctrl.Result{}, err
	}
